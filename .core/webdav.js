...
function handleConditionalAndRange(req, res, headers, size) {
            ...
            (async () => {
              ...
              if (!getImageConversionEnabled()) {
                ...
                const originalHeaders = {
                  ...
                  ETag: '"' + st.size + "-" + Number(st.mtimeMs) + '"',
                  "Accept-Ranges": "bytes",
                  Connection: "Keep-Alive",
                  "Keep-Alive": "timeout=600",
                };

                const conditional = handleConditionalAndRange(
                  req,
                  res,
                  originalHeaders,
                  st.size
                );
                if (conditional.handled) return;

                if (conditional.range) {
                  const { start, end } = conditional.range;
                  res.writeHead(206, {
                    ...originalHeaders,
                  ...
                  fileStream.on("error", (err) => {
                    logger.error(
                      `[元画像Range送信失敗] ${displayPath}: ${err.message}`
                    );
                    if (!res.headersSent) res.writeHead(500);
                    res.end("Failed to read original image");
                  });
                  fileStream.on("end", () => {
                    recordImageStats(end - start + 1, false);
                    logger.info(
                      `[通信量] ${displayPath} - 送信: ${((end - start + 1) / 1024).toFixed(1)} KB (元画像Range)`
                    );
                  });
                  return;
                }

                res.writeHead(200, originalHeaders);
              ...
              if (memoryCachedData) {
                ...
                const headers = {
                  ...
                  "Last-Modified": new Date(st.mtimeMs).toUTCString(),
                  ETag: '"' + key + "-" + memoryCachedData.length + '"',
                  "Accept-Ranges": "bytes",
                  Connection: "Keep-Alive",
                  "Keep-Alive": "timeout=600",
                };
                ...
                if (conditional.range) {
                  const { start, end } = conditional.range;
                  const partial = memoryCachedData.subarray(start, end + 1);
                  res.writeHead(206, {
                    ...headers,
                    "Content-Length": partial.length,
                    "Content-Range": `bytes ${start}-${end}/${memoryCachedData.length}`,
                  });
                  res.end(partial);
                  recordImageStats(partial.length, true);
                  logger.info(
                    `[通信量] ${displayPath} - 送信: ${(partial.length / 1024).toFixed(1)} KB (元: ${(st.size / 1024).toFixed(1)} KB, メモリキャッシュRange)`
                  );
                  return;
                }
                ...
                try {
                  ...
                  if (cst && cst.isFile && cst.isFile()) {
                    ...
                    const headers = {
                      ...
                      "Last-Modified": new Date(st.mtimeMs).toUTCString(), // 最終更新時間を設定
                      ETag: '"' + key + "-" + cst.size + '"', // ETagを設定
                      "Accept-Ranges": "bytes",
                    ...
                    try {
                      ...
                      if (range) {
                        res.writeHead(206, {
                          ...
                          "Content-Range": `bytes ${range.start}-${range.end}/${cst.size}`,
                        });
                      } else {
                        res.writeHead(200, headers); // ステータスコードを200に設定してヘッダーを送信
                      }
                      const s = fs.createReadStream(cachePath, streamOptions);
                      let recorded = false;
                      const recordOnce = () => {
                        if (recorded) return;
                        recorded = true;
                        recordImageStats(responseBytes, true);
                      };
                      const onErr = (e) => {
                      ...
                      s.on("end", () => {
                        recordOnce();
                        logger.info(
                          `[通信量] ${displayPath} - 送信: ${(responseBytes / 1024).toFixed(1)} KB (元: ${(st.size / 1024).toFixed(1)} KB, 圧縮率: ${((1 - responseBytes / st.size) * 100).toFixed(1)}%, ファイルキャッシュ${range ? "Range" : ""})`
                        );
                      });
                      res.on("close", () => {
                        try {
          ...
          try {
            ...
            if (
              ...
              if (supportsGzip) {
                ...
                const writePendingHead = (headers = {}) => {
                  const mergedHeaders = { ...pendingHeaders, ...headers };
                  if (pendingStatusMessage !== undefined) {
                ...
                res.writeHead = function (statusCode, statusMessage, headers) {
                  ...
                  if (isTextResponse && !isImageResponse) {
                    ...
                    pendingHeaders["Vary"] = "Accept-Encoding"; // Varyヘッダーを設定
                  }

                  return this; // end時に圧縮可否を決めてからヘッダーを書き込む
                };
                ...
                res.end = function (chunk, encoding) {
                  ...
                  if (responseData.length === 0) {
                    // レスポンスデータが空の場合
                    if (!res.headersSent) writePendingHead();
                  ...
                  const contentType =
                    getHeaderValue(pendingHeaders, "content-type") || ""; // コンテントタイプを取得
                  const isTextResponse =
                    contentType.includes("xml") || // xmlが含まれているかどうか
                    contentType.includes("html") || // htmlが含まれているかどうか
                    contentType.includes("json") || // jsonが含まれているかどうか
                  ...
                  zlib.gzip(
                    ...
                    (err, compressed) => {
                      ...
                      if (compressionRatio < threshold) {
                        ...
                        if (!res.headersSent) writePendingHead(gzipHeaders);
