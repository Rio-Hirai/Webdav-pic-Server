  ...
  server.setFileSystem("/", new CachedFileSystem(ROOT_PATH, dirCache, statCache), (success) => {
    ...
    const httpServer = http.createServer(async (req, res) => {
      ...
      if (req.method === "PROPFIND") {
        ...
        logger.info(`[PROPFIND] from=${req.connection.remoteAddress} path=${displayPath} depth=${depth}`);
      }

      logger.info(`${req.connection.remoteAddress} ${req.method} ${displayPath}`);
      ...
      if (req.method === "GET" && IMAGE_EXTS.includes(ext)) {
        ...
        requestStack.push({
          ...
          processor: async () => {
            ...
            const key = crypto
              .createHash("md5")
              .update(fullPath + "|" + (getPhotoSize() ?? "o") + "|" + quality + "|" + String(st.mtimeMs) + "|" + String(st.size))
              .digest("hex");
            const cachePath = activeCacheDir ? path.join(activeCacheDir, key + ".webp") : null;
          ...
          if (supportsGzip) {
            ...
            res.end = function(chunk, encoding) {
              ...
              if (!isTextResponse || fullData.length < MIN_COMPRESS_SIZE) {
                logger.info(`[圧縮スキップ] ${displayPath} - 条件未満: テキスト=${isTextResponse}, サイズ=${fullData.length}`);
              ...
              zlib.gzip(fullData, {
                ...
                if (err) {
                  logger.warn(`[圧縮エラー] ${displayPath}: ${err.message}`);
                  return originalEnd.call(this, fullData);
                }
                ...
                logger.info(`[圧縮結果] ${displayPath} - 圧縮率: ${(compressionRatio * 100).toFixed(1)}%, 閾値: ${(threshold * 100).toFixed(1)}%`);
                ...
                if (compressionRatio < threshold) {
                  ...
                  logger.info(`[圧縮適用] ${displayPath} サイズ: ${fullData.length} → ${compressed.length} bytes (圧縮率: ${(compressionRatio * 100).toFixed(1)}%)`);
                  originalEnd.call(this, compressed);
                } else {
                  logger.info(`[圧縮スキップ] ${displayPath} - 圧縮効果が不十分: ${(compressionRatio * 100).toFixed(1)}% >= ${(threshold * 100).toFixed(1)}%`);
            ...
            try {
              ...
              if (fileStat && fileStat.isFile() && fileStat.size > 1024) { // 1KB以上の場合のみ圧縮
                ...
                if (compressionRatio < getCompressionThreshold()) {
                  ...
                  res.end(compressed);
                  logger.info(`[ファイル圧縮完了] ${displayPath} サイズ: ${fileData.length} → ${compressed.length} bytes (圧縮率: ${(compressionRatio * 100).toFixed(1)}%)`);
                  return; // 圧縮レスポンスを送信して処理終了
                }
              }
            } catch (compressError) {
              logger.warn(`[ファイル圧縮エラー] ${displayPath}: ${compressError.message}`);
            }
          }
        }
    ...
    httpServer.on("error", (err) => {
      if (err && err.code === "EADDRINUSE") {
        logger.error(`ポート ${PORT} は既に使用されています。サーバーの起動をスキップします。`);
      } else {
    ...
    httpServer.listen(PORT, () => {
      ...
      logger.info(`[INFO] キャッシュDir=${activeCacheDir || "無効"} / MAX_LIST=${getMaxList()} / Photo_Size=${getPhotoSize() ?? "オリジナル"}`);
      logger.info(`[INFO] 圧縮機能=${getCompressionEnabled() ? "有効" : "無効"} / 圧縮閾値=${(getCompressionThreshold() * 100).toFixed(1)}%`);
    });
  });
}
