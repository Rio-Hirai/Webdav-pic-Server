...
|------|-------------------|------------------------------|
| **転送速度** | ✅ 高速（生ファイルを直接転送） | ⚠️ 変換の処理時間が発生（約200ms）<br>※変換後のファイルサイズ削減により低速回線では逆転する場合あり |
| **プロトコルオーバーヘッド** | ✅ 小さい（バイナリプロトコル、ヘッダー数十〜数百バイト） | ⚠️ やや大きい（HTTP ベース、ヘッダー数百バイト〜数KB）<br>※データサイズ削減により相対的な影響は小さい |
| **通信量** | ❌ 多い（元画像サイズをそのまま転送） | ✅ 大幅削減（WebP 変換により 60～90％削減）|
| **元ファイルへの影響** | ⚠️ 読み書き可能（誤操作のリスクあり） | ✅ **読み取り専用**（HTTP レスポンスのみ変換、元ファイルは保護） |
| **ストレージ容量** | - | ✅ **不要**（変換後画像を保存せず、リアルタイム変換・ストリーミング送信）<br>※オプションのキャッシュ機能を除く |
...
- **真の並列処理制御**: p-limitによる真の並列処理で、複数画像を同時に変換可能（設定値に応じて最大32並列）
...
- 🚀 **真の並列処理**: 画像変換を真に並列実行し、パフォーマンスを大幅向上
...
- **並列処理制限**: `.core/image.js` - `convertAndRespondWithLimit`関数で真の並列数制御とin-flight管理
...
- **真の並列実行**: 複数の画像を同時に変換可能（設定値に応じて最大32並列）
...
- **並列制御**: `.core/image.js` - `pLimit`による真の並列処理制御（並列数制限、in-flight管理）
- **動的再初期化**: `.core/image.js` - `reinitializeConcurrency`関数で設定変更時の並列制御再初期化
- **設定監視**: `main.js` - 設定変更検出とSharp設定の再適用
- **SimpleServerMonitor**: `.core/stack.js` - 軽量監視機能（リクエストカウント、スタック状況ログ）
...
- **並列処理制限**: `main.js` 94-110 行 - Sharpの動的並列度制御（メモリ枯渇と CPU 過負荷の防止）
...
- **真の並列実行**: p-limitによる真の並列処理で、複数画像を同時に変換
...
```コマンドライン
プロジェクトルート/
├── main.js (188行) - メインエントリーポイント
├── config.txt - 動的設定ファイル
├── .core/
│   ├── config.js (460行) - 設定管理・検証・動的読み込み
│   ├── cache.js (148行) - キャッシュシステム管理
│   ├── stack.js (299行) - スタック処理システム（未使用）
│   ├── image.js (1331行) - 画像変換処理
│   └── webdav.js (1025行) - WebDAVサーバー実装
...
- SimpleServerMonitorクラス（軽量監視機能）
- 注: 現在は画像変換には使用されていない（並列処理に移行）
...
| `convertAndRespondWithLimit()` | .core/image.js | 並列制限付き画像変換（p-limitによる真の並列処理、in-flight管理、重複防止） |
...
- **v24.3.0**: 並列処理の実装修正
  - 🚀 **真の並列処理**: 画像変換のスタック処理を除去し、p-limitによる真の並列処理を実装
